four51.app.controller('ProductCtrl', ['$scope', '$routeParams', '$route', '$location', '$451', 'Category', 'Nav', 'Product', 'ProductDisplayService', 'Order', 'Variant', 'User', 'Lightbox',
	function ($scope, $routeParams, $route, $location, $451, Category, Nav, Product, ProductDisplayService, Order, Variant, User, Lightbox) {
		...
		$scope.createNewVariant = true;
		$scope.$watch('LineItem.Product.StaticSpecGroups.CreateNewVariant.Specs.Allow.Value', function(val) {
			if (!val) return;
			if($scope.LineItem.Product.StaticSpecGroups.CreateNewVariant.Specs.Allow.Value == "No"){
				$scope.createNewVariant = false;
			}
		});

		$scope.showVarSpecsOnMatrix = false;
		$scope.$watch('LineItem.Product.StaticSpecGroups.VarSpecsOnMatrix.Specs.Show.Value', function(val) {
			if (!val) {
				return;
			}
			if($scope.LineItem.Product.StaticSpecGroups.VarSpecsOnMatrix.Specs.Show.Value == "Yes"){
				$scope.showVarSpecsOnMatrix = true;
			}
		});
		...
		$scope.openLightboxModal = function (index) {
			if($scope.LineItem.Variant){
				$scope.LineItem.images[0].thumbUrl = $scope.LineItem.Variant.LargeImageUrl;
				$scope.LineItem.images[0].url = $scope.LineItem.Variant.LargeImageUrl;
